...
func newTestPolicyEngine(t *testing.T) (*policy.ConfigPolicyEngine, *store.SQLiteStore) {
	...
	pe := policy.NewConfigPolicyEngineFromConfig(config, s)
	return pe, s
}
...
// matchResource replicates the unexported matchResource logic from the policy package
// for computing expected authorization results in property tests.
...
// containsAction replicates the unexported containsAction logic from the policy package.
...
func TestProperty14_AuthorizationAndAudit(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		// Compute expected result using the same logic as the policy package
...
func TestProperty15_ToolWhitelistAndRateLimit(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		// Compute expected result using the same whitelist config
		toolWhitelist := map[string][]string{
			"developer_agent": {"llm_api", "file_write", "file_read"},
			"readonly_agent":  {"llm_api", "file_read"},
		}
		expectedAllowed := false
		for _, at := range toolWhitelist[role] {
			if at == tool {
				expectedAllowed = true
				break
			}
