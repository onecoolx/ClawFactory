...
// testToken is the token used for authenticating test requests through the router.
const testToken = "test-token-for-task-tests"

// mockPolicyEngineForTask implements policy.PolicyEngine.
// ShouldRetry reads the task's retry_count from the store and compares with maxRetries.
type mockPolicyEngineForTask struct {
	store      store.StateStore
	maxRetries int
}

func (m *mockPolicyEngineForTask) CanExecuteTask(agentID string, taskCapabilities []string) bool {
	return true
}
func (m *mockPolicyEngineForTask) Authorize(req model.AuthorizeRequest) model.AuthorizeResponse {
	return model.AuthorizeResponse{Allowed: true}
}
func (m *mockPolicyEngineForTask) ShouldRetry(taskID string) (bool, error) {
...
func (m *mockPolicyEngineForTask) GetMaxRetries() int { return m.maxRetries }
func (m *mockPolicyEngineForTask) IsToolAllowed(agentID string, toolName string) bool {
	return true
}
func (m *mockPolicyEngineForTask) CheckRateLimit(agentID string, toolName string) bool {
	return true
}

// mockWorkflowEngineForTask implements workflow.WorkflowEngine.
// Tracks calls to OnTaskPermanentlyFailed for verification.
type mockWorkflowEngineForTask struct {
	mu                     sync.Mutex
	permanentlyFailedTasks []string
}

func (m *mockWorkflowEngineForTask) SubmitWorkflow(def model.WorkflowDefinition) (model.WorkflowInstance, error) {
	return model.WorkflowInstance{}, nil
}
func (m *mockWorkflowEngineForTask) ValidateDAG(def model.WorkflowDefinition) error { return nil }
func (m *mockWorkflowEngineForTask) OnTaskCompleted(taskID string) error            { return nil }
func (m *mockWorkflowEngineForTask) OnTaskPermanentlyFailed(taskID string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.permanentlyFailedTasks = append(m.permanentlyFailedTasks, taskID)
	return nil
}
func (m *mockWorkflowEngineForTask) GetWorkflowStatus(instanceID string) (model.WorkflowInstance, error) {
	return model.WorkflowInstance{}, nil
}
func (m *mockWorkflowEngineForTask) wasPermanentlyFailed(taskID string) bool {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockWorkflowEngineForTask) reset() {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// newTestStoreForTaskRapid creates a temp SQLite store for use inside rapid.Check.
// Returns the store and a cleanup function that must be called via defer.
func newTestStoreForTaskRapid(rt *rapid.T) (*store.SQLiteStore, func()) {
	tmpFile, err := os.CreateTemp("", "clawfactory-api-task-test-*.db")
	if err != nil {
		rt.Fatal(err)
	}
	tmpFile.Close()
...
// setupTestRouterForTask creates a full chi router via api.NewRouter with a known test token.
// This routes through the public /v1/tasks/{taskID}/status endpoint with TokenAuthMiddleware.
func setupTestRouterForTask(srv *api.Server) http.Handler {
	return api.NewRouter(srv, []string{testToken}, metrics.NewNoopCollector(), false)
}

// sendFailedStatusWithAuth sends an authenticated POST request to update task status to "failed".
func sendFailedStatusWithAuth(router http.Handler, taskID string) *httptest.ResponseRecorder {
...
// ensureWorkflowForTask creates a workflow instance for foreign key constraints.
func ensureWorkflowForTask(rt *rapid.T, s store.StateStore, wfID string) {
...
func TestProperty30_RetryVsPermanentFailure(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		s, cleanup := newTestStoreForTaskRapid(rt)
		defer cleanup()
		queue := taskqueue.NewStoreBackedQueue(s)
		mockWf := &mockWorkflowEngineForTask{}
		mockPolicy := &mockPolicyEngineForTask{store: s, maxRetries: maxRetries}
		...
		router := setupTestRouterForTask(srv)
		...
		ensureWorkflowForTask(rt, s, wfID)
		...
		rec := sendFailedStatusWithAuth(router, taskID)
