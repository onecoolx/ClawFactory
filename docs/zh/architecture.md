...
- `GET /metrics` — Prometheus 指标（无需认证，v0.3 新增）
...
- `GET /v1/admin/webhooks` — 列出 Webhook 订阅（v0.3 新增）
...
v0.3.1 新增事务支持方法（`RunInTransaction`、`RequeueTaskTx`、`RetryTaskTx`），确保关键操作的原子性。

## 平台运行特性

### 优雅关闭（v0.3.1 新增）

平台支持优雅关闭：接收到 SIGINT 或 SIGTERM 信号后，停止接受新请求，等待正在处理的请求完成，停止心跳检查 goroutine，关闭数据库连接，输出结构化关闭日志。
